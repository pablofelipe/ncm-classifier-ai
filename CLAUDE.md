...
- **ADR-0017** — `NCMCode` value object (`src/core/domain/ncm.py`): validates the dotted `XXXX.XX.XX` format at construction, exposes `.dotless` and `.matches_heading()`. Wired into `ClassificationCandidate.ncm_code`, `TIPIIndex` (keys + `verify()`), `HybridRetrievalAdapter`'s RRF fusion dict, all four retrieval adapters, `GenericLLMRerankAdapter` (defensive parsing), and the HTTP/eval boundaries (converted back to `str` at the edge). Closes the primitive-obsession gap flagged in the architecture review, removes three manual `.replace(".", "")` conversions, and closes a real `NCMCandidate.ncm` validation gap (now format-checked). Surfaced two non-cosmetic bugs during wiring — `routes.py` passing `NCMCode` into a Pydantic `str` field (runtime `ValidationError`, invisible to `mypy`) and `eval/run_eval.py` silently zeroing every hit by comparing `NCMCode` against `str` — both fixed with an explicit `str(...)` at the respective boundary. Pure refactor: v1 CI baseline (33.3%/63.3%) and the v2 hybrid baseline (49.1%/68.0%) both reproduced exactly after rebuilding each index (see `docs/adr/0017-ncm-code-value-object.md`).
